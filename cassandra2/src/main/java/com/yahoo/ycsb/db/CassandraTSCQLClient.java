 ...
 * @author cmatser
 */
public class CassandraTSCQLClient extends DB {
  ...
  public static final String QUANTA_SIZE_PROPERTY = "cassandra.quanta_size";
  private static long quantaSize;
  ...
  public void init() throws DBException {
    ...
    synchronized (INIT_COUNT) {
      ...
      try {
        ...
        quantaSize = Long.parseLong(getProperties().getProperty(QUANTA_SIZE_PROPERTY, "900"));
  ...
  public Status read(String table, String key, Set<String> fields,
    ...
    try {
      ...
      stmt = selectBuilder.from(table)
    	  .where(QueryBuilder.eq("host", host))
    	  .and(QueryBuilder.eq("worker", workerName))
    	  .and(QueryBuilder.eq("quanta", quanta))
    	  .and(QueryBuilder.eq("time", timestamp))
          .limit(1);
  ...
  private long calculateQuanta(long timestamp) {
	  long diff = timestamp % quantaSize;
	  long q = timestamp - diff;
	  return q;
  }
